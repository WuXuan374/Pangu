...
if __name__ == '__main__':
    print(get_canonical_lisp("(AND measurement_unit.unit_of_frequency (JOIN measurement_unit.unit_of_frequency.measurement_system (JOIN measurement_unit.measurement_system.current_density_units g.122rvtfy)))"))
    print(get_canonical_lisp("(AND measurement_unit.measurement_system (JOIN measurement_unit.measurement_system.length_units m.01464h))"))
    print(get_canonical_lisp("(ARGMAX measurement_unit.measurement_system (JOIN measurement_unit.measurement_system.time_units measurement_unit.time_unit.time_in_seconds))"))
    print(get_canonical_lisp("(AND computer.software (AND (JOIN computer.software.software_genre m.05vvqy) (JOIN computer.software.software_genre m.0h2vrf)))"))

    exit(0)

    print(max_count_relations("(JOIN influence.influence_node.influence (JOIN influence.influence_node.influenced (JOIN influence.influence_node.influenced_by (JOIN influence.influence_node.influenced (JOIN influence.influence_node.influenced_by m.060nc)))))"))

    exit(0)
    print(same_logical_form(
        "(AND measurement_unit (JOIN measurement_unit.measurement_system.solid_angle_units m.0708n))",
        "(AND measurement_unit.measurement_system (JOIN measurement_unit.measurement_system.solid_angle_units m.0708n))"))
    exit(0)

    print(lisp_to_nested_expression(''))
    exit(0)
    print(postprocess_raw_code("(ARGMIN boats.ship_designer boats.ship.designer_inv boats.ship.length_at_waterline)"))
    exit(0)
    # program = '(AND book.journal (JOIN book.periodical.editorial_staff (AND (JOIN book.editorial_tenure.editor m.026_1m5) (JOIN book.editorial_tenure.title m.02h6676))))'
    # expression = lisp_to_nested_expression(program)
    # derivations = get_derivations_from_lisp(expression)
    # print(derivations)
    program = "(ARGMAX tennis.tennis_match_type (JOIN (R tennis.tennis_match.match_format) tennis.tennis_match.date))"
    sparql = lisp_to_sparql(program)
    print(sparql)
    print(execute_query(sparql))
